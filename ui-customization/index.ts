...
function gradientText(text: string, row: number, totalRows: number) {
  const characters = [...text];
  const maxLineWidth = 16; // Largura do logo
  
  return characters
    .map((character, col) => {
      if (character === " ") return character;
      // Gradiente diagonal: combina posição X e Y
      const x = col / maxLineWidth;
      const y = row / totalRows;
      const position = (x + y) / 2; // Média para gradiente diagonal
      return foreground(sampleGradient(position), character);
    })
    .join("");
}
...
export default function uiCustomization(pi: ExtensionAPI) {
  ...
  function install(ctx: ExtensionContext) {
      ...
      return {
        render(width: number) {
          const art = TITLE_LINES.map((line, row) =>
            truncateToWidth(gradientText(line, row, TITLE_LINES.length), width),
          );
    ...
    // Footer not customized - using pi original footer
